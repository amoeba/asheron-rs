...
pub fn generate_message_parser(
    ...
    out.push_str("use std::io::{Cursor, Seek};\n");
    ...
    out.push_str("    cursor: &mut Cursor<&[u8]>,\n");
    out.push_str(") -> Result<serde_json::Value, Box<dyn std::error::Error>> {\n");
    out.push_str("    // Store initial position before reading header\n");
    out.push_str("    let _initial_pos = cursor.position();\n");
    out.push_str("    \n");
    out.push_str("    // Read header fields to determine the event type (as per Chorizite pattern)\n");
    out.push_str("    let object_id = crate::readers::read_u32(&mut Box::new(cursor.clone()))?;\n");
    out.push_str("    let sequence = crate::readers::read_u32(&mut Box::new(cursor.clone()))?;\n");
    out.push_str("    let event_type_value = crate::readers::read_u32(&mut Box::new(cursor.clone()))?;\n");
    out.push_str("    \n");
    ...
    out.push_str("        })),\n");
    out.push_str("    };\n\n");
    out.push_str("    // Backtrack to allow the specific struct to read the full payload (as per Chorizite pattern)\n");
    out.push_str("    let pos = cursor.position();\n");
    out.push_str("    if pos >= 12 {\n");
    out.push_str("        cursor.seek(std::io::SeekFrom::Current(-12))?;  // Backtrack 3 u32s = 12 bytes\n");
    out.push_str("    } else {\n");
    out.push_str("        cursor.set_position(0);\n");
    out.push_str("    }\n");
    out.push_str("    \n");
    out.push_str("    // Create a reader from the current cursor position to parse the full payload\n");
    out.push_str("    let mut reader: Box<dyn ACReader> = Box::new(cursor.clone());\n");
    out.push_str("    \n");
    out.push_str("    // Parse using the specific event struct (which will read header + payload)\n");
    ...
    out.push_str("    let _initial_pos = cursor.position();\n");
    out.push_str("    \n");
    out.push_str("    // Read header fields to determine the action type (as per Chorizite pattern)\n");
    out.push_str("    let sequence = crate::readers::read_u32(&mut Box::new(cursor.clone()))?;\n");
    out.push_str("    let action_type_value = crate::readers::read_u32(&mut Box::new(cursor.clone()))?;\n");
    out.push_str("    \n");
    ...
    out.push_str("    // Backtrack to allow the specific struct to read the full payload (as per Chorizite pattern)\n");
    out.push_str("    let pos = cursor.position();\n");
    out.push_str("    if pos >= 8 {\n");
    out.push_str("        cursor.seek(std::io::SeekFrom::Current(-8))?;  // Backtrack 2 u32s = 8 bytes\n");
    out.push_str("    } else {\n");
    out.push_str("        cursor.set_position(0);\n");
    out.push_str("    }\n");
    out.push_str("    \n");
    ...
    out.push_str("    let mut reader: Box<dyn ACReader> = Box::new(cursor.clone());\n");
    out.push_str("    \n");
    out.push_str("    // Parse using the specific action struct (which will read header + payload)\n");
