    ...
    pub fn read(reader: &mut dyn ACReader) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        match type_ {
            TurbineChatType::ServerToClientMessage => {
                let variant_struct = CommunicationTurbineChatType1::read(reader, message_size, target_type, target_id, transport_type, transport_id, cookie, payload_size)?;
                Ok(Self::Type1(variant_struct))
            },
            TurbineChatType::ClientToServerMessage => {
                let variant_struct = CommunicationTurbineChatType3::read(reader, message_size, target_type, target_id, transport_type, transport_id, cookie, payload_size)?;
                Ok(Self::Type3(variant_struct))
            },
            TurbineChatType::AckClientToServerMessage => {
                let variant_struct = CommunicationTurbineChatType5::read(reader, message_size, target_type, target_id, transport_type, transport_id, cookie, payload_size)?;
